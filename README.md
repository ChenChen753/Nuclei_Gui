...
- Windows：上传 `Nuclei_GUI*.exe`，例如 `Nuclei_GUI_v2.5.4.exe` 或 `Nuclei_GUI_windows_x64_v2.5.4.exe`；当前二进制更新策略仅面向 Windows exe。
...
- **DNSLog/OAST**: 自动识别 Interactsh 模板，支持旧 DNSLog 占位符兼容
- **更新设置**: 自动检查更新、手动更新
...
支持基于 Nuclei 原生 Interactsh 的 OAST/DNSLog 验证：

- **自动模式**：选中的 POC 使用 `{{interactsh-url}}`、`interactsh_protocol` 等变量时自动启用
- **关闭模式**：禁用 Interactsh/OAST，适用于不希望产生外联请求的场景
- **强制开启**：对当前扫描强制追加 Interactsh 参数
- **自建服务**：可配置 Interactsh Server 和 Token
- **旧占位符兼容**：可将 `{{dnslog}}`、`{{callback_url}}` 等旧 DNSLog 占位符临时转换为 `{{interactsh-url}}`

旧占位符转换只生成临时模板副本，不会修改原始 POC 文件。
...
本工具仅供网络安全学习、研究、内部安全建设和已获得明确授权的安全测试使用。

使用者在使用本工具前，应确保已经获得被测试目标系统、资产、域名、IP、应用或数据所有者的明确授权，并遵守所在国家或地区的法律法规、行业规范和授权范围。

严禁将本工具用于以下场景：

- 未经授权的漏洞扫描、渗透测试、资产探测或批量检测
- 攻击、入侵、破坏、窃取数据、绕过访问控制或影响第三方系统正常运行
- 针对公共互联网目标进行未授权扫描或高频请求
- 任何违反法律法规、服务协议或授权边界的行为

本项目作者和贡献者不对任何滥用、误用、未授权使用或由此造成的直接或间接损失承担责任。下载、安装、运行或二次分发本工具，即表示你理解并同意自行承担全部使用风险和法律责任。
...
### v2.5.4 (当前版本)
- ✅ **exe 更新兜底修复**：修复 Windows exe 已拿到 exe 下载地址但仍按源码 zip 解压导致 `File is not a zip file` 的问题
- ✅ **更新弹窗显示修复**：修复自动更新提示中 `\n\n` 显示为文本、中文界面按钮仍为 Yes/No 的问题
...
### v2.5.0
- ✅ **双语完善**：修复多处中英文界面、弹窗、状态、扫描记录和报告导出的语言不一致问题
- ✅ **界面自适应优化**：调整高分辨率/DPI 场景下的按钮、间距、边框和设置页布局
- ✅ **Nuclei 下载修复**：修复内置下载 Nuclei 功能，并保持二进制文件不纳入仓库
- ✅ **扫描暂停/继续修复**：修复扫描页和任务管理中的暂停、继续逻辑
- ✅ **目标去重**：手动输入、文件导入、FOFA 导入、任务队列和扫描线程统一进行目标规范化与去重
- ✅ **DNSLog/OAST 支持**：新增独立 DNSLog 设置页，自动识别 `{{interactsh-url}}` 模板并支持旧 DNSLog 占位符临时适配
- ✅ **报告导出语言修复**：英文界面导出的报告默认使用英文文案
- ✅ **数据库清理增强**：清空扫描、AI、FOFA 历史时同步清理 SQLite 数据表并执行压缩，减少历史数据残留
- ✅ **仪表盘优化**：修复扫描历史状态、清空确认弹窗和记录按钮显示问题
